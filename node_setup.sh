...
HOME="${WORKINGDIR}"
DEPLOY_CONFIG="${WORKINGDIR}/cloudlab_profiles/$K8SVERSION/kube-deploy-yaml/"
...
EOF
  ...
  #sudo chown ${username}:${usergid} $HOME/config
  #sudo chmod g+r $HOME/config
  cd $WORKINGDIR
  export HOME="/mnt/extra/"
  sudo cp /etc/kubernetes/admin.conf $HOME/
  sudo chown ${username}:${usergid} $HOME/admin.conf
  ...
  kubectl apply -f kube_deploy_yaml/kube-flannel.yml ### Changed to specify the IFace
  kubectl apply -f kube_deploy_yaml/create-cluster-role-binding-admin.yaml
  kubectl apply -f kube_deploy_yaml/create-service-account-admin-uesr-dashboard.yaml
...
LIBJEMALLOC='../deps/jemalloc/lib/libjemalloc.a'
cd $DIR_RFT
sudo docker build -f rmr_base.df -t rmr:ubuntu .
cd 'libs' && wget http://download.redis.io/releases/redis-6.0.5.tar.gz
tar xzf redis-6.0.5.tar.gz && cd $REDIS
sudo make
echo 'make finished'
cd 'src' && ar rcvs $LIBREDIS *.o
cp $LIBREDIS $DIR_RFT'/libs'
cp $LIBHIREDIS $DIR_RFT'/libs'
cp $LIBJEMALLOC $DIR_RFT'/libs'
cd $DIR_RFT
mkdir 'logs'
sudo ./build.sh
echo 'build finished'
